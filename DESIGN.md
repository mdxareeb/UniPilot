...
Glass is for shell-level surfaces; Solid is the default content-card fill. Glass applied to many small elements is a defect — use Solid for content cards. Nested surfaces never use shadow for separation (see below); nested inside Inverted they step lighter; nested inside Solid or Glass they take a subtle fill step.
